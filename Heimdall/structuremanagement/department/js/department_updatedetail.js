function department_updatedetail(api_data_url){
                ...
                if(dataset){
                    ...
                    if(dataset.picture){
                        if(dataset.picture.url_file && document.getElementById('id_picture_url_file')){
                            document.getElementById('id_picture_url_file').setAttribute('value',dataset.picture.url_file);
                        };
                        if(dataset.picture.name && dataset.picture.url_detail && document.getElementById('id_picture_url_detail')){
                            document.getElementById('id_picture_url_detail').setAttribute('value',dataset.picture.url_detail);
                            document.getElementById('id_picture_url_detail').innerHTML = dataset.picture.name
                        };
                    };
                    if(dataset.process_instructions){
                        if(dataset.process_instructions.url_file && document.getElementById('id_process_instruction_url_file')){
                            document.getElementById('id_process_instruction_url_file').setAttribute('value',dataset.process_instructions.url_file);
                        };
                        if(dataset.process_instructions.name && dataset.process_instructions.url_detail && document.getElementById('id_process_instruction_url_detail')){
                            document.getElementById('id_process_instruction_url_detail').setAttribute('value',dataset.process_instructions.url_detail);
                            document.getElementById('id_process_instruction_url_detail').innerHTML = dataset.process_instructions.name
                        };
                    };
                    ...
                    if (dataset.sections){
                        if (dataset.sections.count && document.getElementById('id_sections_count')){
                            document.getElementById('id_sections_count').innerHTML = dataset.sections.count
                        };
                    };
                    ...
                    if(dataset.sections.data){
                        let col_sections = [
                            ...
                            },
                            {
                                ...
                                "defaultContent": "",
                                "className":"",
                                "visible": "reference_number" in dataset.sections.data[0] ? true : false,
                            ...
                            },
                            {
                                ...
                                "defaultContent": "",
                                "className":"",
                                "visible": "name" in dataset.sections.data[0] ? true : false,
                                "render":function(data,type,row,meta){
                                        return row.name
                                }
                            },
                            {
                                ...
                                "width":"35px",
                                "className":"",
                                "visible": "url" in dataset.sections.data[0] ? true : false,
                                "render":function(data,type,row,meta){
                                    if (row.url){
                                        if (row.url.detail){
                        ...
                        transformTable("sections",dataset.sections.data,col_sections,[[2,"asc"]]);
                    };
                    if(dataset.positions.data){
                        let col_positions = [
                            ...
                            },
                            {
                                ...
                                "defaultContent": "",
                                "className":"",
                                "visible": "section" in dataset.positions.data[0] ? true : false,
                                "render":function(data,type,row,meta){
